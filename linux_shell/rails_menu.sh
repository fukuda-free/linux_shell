...
FNC_MENU()
{
	...
	echo "  [1]  nginx + unicorn 停止"
	echo "  [2]  nginx + unicorn 起動"
	echo "  [3]  DB + テーブル 構築"
	echo "  [4]  DB + テーブル 再構築"
	echo "  [5]  キャッシュの削除"
	echo "  [6]  ルートファイルの確認"
	echo "  [7]  bundle install 実行"
	echo "  [8]  bundle install 再実行"
	echo "  [9]  rails console 実行"
	echo "  [e]  シェルを終了"
	echo ""
	echo " ---  【 本番環境 (production) 】  ---"
	echo " [100] nginx + unicorn 停止"
	echo " [101] nginx + unicorn 起動"
	echo " [102] DB + テーブル 構築"
	echo " [103] キャッシュの削除"
	echo " [104] コンパイル"
	echo " [105] DB + テーブル 再構築"
	echo ""
	echo " ---  gem  ---"
	echo " [201] rails_best_practices 実行"
	echo " [202] delayed_job 再起動"
	echo " [203] i18n_generators 実行"
	echo " [204] whenever 再設定"
	echo " [205] bower install"
...
NGINX_PRO_START(){
	...
	SECRET_KEY_BASE=$(rake secret) bundle exec unicorn -D -c config/unicorn.rb -E production
